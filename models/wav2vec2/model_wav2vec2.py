...
from transformers.modeling_utils import PreTrainedModel, torch_int_div
    ...
    def forward(
        ...
        if head_mask is not None:
            if head_mask.size() != (self.num_heads,):
                raise ValueError(
                    f"Head mask for a single layer should be of size {(self.num_heads,)}, but is {head_mask.size()}"
                )
...
class Wav2Vec2EncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False, head_mask=None):
